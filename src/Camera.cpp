...
void Camera::update(double dt) {
	...
	_offsetCameraNormal.y = -0.1+std::min(0.5 * _player->getSpeed(), 0.1);
	...
	if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS ) {
		...
		//posCamera = glm::vec3(posCamera.x + ((std::rand() % 3 + 1 - 5/2.f)/200.f), posCamera.y + ((std::rand() % 5 + 1 - 5/2.f)/100.f), posCamera.z);
	} else {
		_timeMove -= dt;
		if (_timeMove < 0)
			_timeMove = 0;
